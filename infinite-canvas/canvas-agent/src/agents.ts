...
export async function listCodexThreads(emit: AgentEmit, options: { cwd: string; searchTerm?: string; limit?: number }) {
    ...
    const data = Array.isArray(field(result, 'data'))
        ? (field(result, 'data') as unknown[]).map(summarizeCodexThread).filter((thread) => threadInWorkspace(thread, options.cwd))
        : []
        ...
        arrayValue(field(turn, 'items')).forEach((item, itemIndex) => {
            ...
            if (type === 'mcpToolCall') {
                ...
                messages.push({
                    ...
                    title: toolName(tool),
                    text: error ? String(error) : `${toolName(tool)} ${String(field(item, 'status') || '完成')}`,
                    detail: item
                })
            }
            if (type === 'commandExecution') {
                ...
                if (command)
                    messages.push({
                        ...
                        title: '命令',
                        text: command,
                        detail: { cwd: field(item, 'cwd'), status: field(item, 'status'), exitCode: field(item, 'exitCode') }
                    })
            }
