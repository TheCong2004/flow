...
export class CanvasSession {
    ...
    async callTool(name: unknown, rawInput: unknown) {
        ...
        if (tool === 'canvas_create_text_nodes') {
            const data = input as {
                items: Array<{ text: string; title?: string; x?: number; y?: number; width?: number; height?: number }>
                x?: number
                y?: number
                gap?: number
        ...
        if (
            ...
            input = {
                ops: generationFlowOps(
                    { ...(input as Record<string, unknown>), mode: tool.replace('canvas_generate_', ''), autoRun: true },
                    this.canvasState
                )
            }
            tool = 'canvas_apply_ops'
