...
export function AssetPickerModal({ open, onInsert, onClose }: Props) {
    return (
        <Modal title="Chọn media" open={open} onCancel={onClose} footer={null} width={860} destroyOnHidden styles={{ body: { padding: "0 24px 24px", minHeight: 480 } }}>
        ...
        >
            ...
            <div className="p-2.5">
                <div className="flex items-center justify-between gap-2">
                    ...
                    <Tag className="m-0 shrink-0 text-[10px]">{kind === "image" ? "Ảnh" : kind === "video" ? "Video" : "Văn bản"}</Tag>
                </div>
            </div>
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center bg-stone-950/0 text-sm font-medium text-white opacity-0 transition group-hover:bg-stone-950/55 group-hover:opacity-100">Chèn</div>
        </button>
    );
}
    ...
    return (
        <div className="space-y-4">
            ...
            {visible.length ? (
                ...
                <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="Chưa có media" className="py-12" />
            )}
