...
const ONLINE_AGENT_TOOLS: ResponseFunctionTool[] = [
    ...
    toolDefinition("canvas_create_text_node", "在当前画布创建单个Văn bản节点。", { text: { type: "string" }, x: { type: "number" }, y: { type: "number" }, title: { type: "string" }, width: { type: "number" }, height: { type: "number" } }),
    toolDefinition(
    ...
    toolDefinition("canvas_create_config_node", "创建Cấu hình tạo节点，可指定 text/image/video/audio 模式和生成参数，可Chọn立即触发生成。", {
        prompt: { type: "string" },
        mode: GENERATION_MODE_SCHEMA,
        title: { type: "string" },
        x: { type: "number" },
    ...
    generationToolDefinition("canvas_create_generation_flow", "创建通用生成流程：提示词Văn bản节点、Cấu hình tạo节点、参考节点连线，可用于文案、生图、Video或Âm thanh。"),
    ...
    toolDefinition("canvas_update_node", "Cập nhật node基础字段或 metadata。", { id: { type: "string" }, patch: JSON_RECORD_SCHEMA, metadata: JSON_RECORD_SCHEMA }, ["id"]),
    toolDefinition("canvas_update_node_text", "更新Văn bản节点内容和标题。", { id: { type: "string" }, text: { type: "string" }, title: { type: "string" } }, ["id", "text"]),
    toolDefinition(
        "canvas_move_nodes",
    ...
    toolDefinition("canvas_delete_nodes", "Xóa指定节点及相关连线。", { ids: { type: "array", items: { type: "string" }, minItems: 1 } }, ["ids"]),
    toolDefinition(
    ...
    toolDefinition("canvas_run_generation", "触发指定节点生成，通常用于配置节点或Văn bản/Ảnh/Video/Âm thanh节点。", { nodeId: { type: "string" }, mode: GENERATION_MODE_SCHEMA, prompt: { type: "string" } }, ["nodeId"]),
];
...
export function CanvasAssistantPanel({
        ...
        try {
            ...
            if (result.toolCalls.length) {
                ...
                if (confirmTools && writableCalls.length) {
                    upsertMessage(sessionId, { id: assistantId, role: "assistant", text: result.content || streamed || "准备执行工具，等待Xác nhận。" });
                    ...
                    const toolMessage: CanvasAssistantMessage = { id: toolMessageId, role: "tool", title: "Xác nhận工具调用", text: summarizeToolCalls(result.toolCalls), detail: { status: "pending", step: loop.step, toolCalls: result.toolCalls } };
    ...
    const continueOnlineToolLoopAfterResults = async (sessionId: string, assistantId: string, messages: ResponseInputMessage[], toolCalls: ResponseToolCall[], toolResults: OnlineExecutedToolCall[], step: number) => {
        ...
        if (next.toolCalls.length) {
            ...
            if (confirmTools && writableCalls.length) {
                upsertMessage(sessionId, { id: assistantId, role: "assistant", text: next.content || streamed || "准备执行工具，等待Xác nhận。" });
                const toolMessageId = nanoid();
                pendingToolContextRef.current.set(toolMessageId, { messages: nextMessages, toolCalls: next.toolCalls, assistantId, step: step + 1 });
                appendMessage(sessionId, { id: toolMessageId, role: "tool", title: "Xác nhận工具调用", text: summarizeToolCalls(next.toolCalls), detail: { status: "pending", step: step + 1, toolCalls: next.toolCalls } });
    ...
    const rejectOnlineTool = (messageId: string) => {
        ...
        pendingToolContextRef.current.delete(messageId);
        if (session) upsertMessage(session.id, { id: messageId, role: "tool", title: "已拒绝执行", text: "工具调用已Hủy", detail: { ...objectDetail(session.messages.find((item) => item.id === messageId)?.detail), status: "rejected" } });
    };
...
function resolveModelIcon(model: string) {
    ...
    if (name.includes("grok")) {
        try {
            return typeof chrome !== "undefined" && chrome.runtime?.getURL ? chrome.runtime.getURL("assets/tobyflow/dist/infinite-canvas/icons/grok.svg") : new URL("dist/infinite-canvas/icons/grok.svg", document.baseURI).href;
        } catch {
...
function describeCanvasSnapshot(snapshot: CanvasAgentSnapshot) {
    ...
    return `当前画布有 ${snapshot.nodes.length} 个节点、${snapshot.connections.length} 条连线。Văn bản ${counts[CanvasNodeType.Text] || 0} 个，Ảnh ${counts[CanvasNodeType.Image] || 0} 个，Cấu hình tạo ${counts[CanvasNodeType.Config] || 0} 个，Video ${
...
function explainNoop(ops: CanvasAgentOp[], snapshot: CanvasAgentSnapshot) {
    ...
    if (deleteConnectionOps.length && deleteConnectionOps.every((op) => !op.all && [...(op.ids || []), ...(op.id ? [op.id] : [])].every((id) => !connectionIds.has(id)))) return "没有找到要Xóa的连线。";
    ...
    if (deleteNodeOps.length && deleteNodeOps.every((op) => op.nodeType === CanvasNodeType.Config) && !snapshot.nodes.some((node) => node.type === CanvasNodeType.Config)) return "画布当前没有Cấu hình tạo节点可Xóa。";
    if (deleteNodeOps.length && deleteNodeOps.every((op) => [...(op.ids || []), ...(op.id ? [op.id] : [])].every((id) => !nodeIds.has(id)))) return "没有找到要Xóa的节点。";
