        ...
        >
            <div className="mb-2 flex items-center justify-between gap-2">
                <div className="flex min-w-0 items-baseline gap-2">
                    <div className="shrink-0 text-xs font-semibold">Ghép prompt</div>
                    <div className="truncate text-[11px] opacity-55">Dùng @ để tham chiếu tài nguyên đã nối, tự đánh số lại trước khi gửi</div>
                </div>
            ...
            <div className="relative rounded-xl border" style={{ background: theme.node.fill, borderColor: theme.node.stroke }}>
                {!value.trim() ? (
                    <div className="pointer-events-none absolute left-3 top-2 text-sm leading-7" style={{ color: theme.node.placeholder }}>
                        Nhập prompt, dùng @ để tham chiếu ảnh hoặc văn bản đã nối
                    </div>
                ) : null}
                <div
                    ref={editorRef}
            ...
            {imagePreview ? <Image src={imagePreview} alt="Xem trước ảnh tham chiếu" style={{ display: "none" }} preview={{ visible: true, src: imagePreview, onVisibleChange: (visible) => !visible && setImagePreview(null) }} /> : null}
        </div>
    );
}
...
function resourceLabel(input: NodeGenerationInput, inputs: NodeGenerationInput[]) {
    ...
    if (input.type === "image") return `Ảnh ${index + 1}`;
    if (input.type === "video") return `Video ${index + 1}`;
    if (input.type === "audio") return `Âm thanh ${index + 1}`;
