...
export function CanvasNodeHoverToolbar({
    ...
    const copyImagePrompt = (target: CanvasNodeData) => {
        ...
        if (!prompt) {
            message.warning("Chưa có prompt để sao chép");
            return;
        }
        copyText(prompt, "Đã sao chép prompt");
    };
    ...
    const baseToolbarTools: ToolbarTool[] = [
        { id: "info", title: "Xem thông tin node", label: "Thông tin", icon: <Info className="size-4" />, onClick: () => onInfo(node) },
        { id: "delete", title: "Xóa node", label: "Xóa", icon: <Trash2 className="size-4" />, onClick: () => onDelete(node), danger: true },
    ];
    const nodeToolbarTools: ToolbarTool[] = [
        ...(canRetry ? [{ id: "retry", title: "Tạo lại", label: "Thử lại", icon: <RefreshCw className="size-4" />, onClick: () => onRetry(node) }] : []),
        ...(hasImage || hasVideo || isText ? [{ id: "saveAsset", title: "Thêm vào tài nguyên của tôi", label: "Lưu", icon: <FolderPlus className="size-4" />, onClick: () => onSaveAsset(node) }] : []),
        ...(hasImage || hasVideo || hasAudio ? [{ id: "download", title: hasAudio ? "Tải âm thanh" : hasVideo ? "Tải video" : "Tải ảnh", label: "Tải", icon: <Download className="size-4" />, onClick: () => onDownload(node) }] : []),
        ...(canOpenDialog ? [{ id: "edit", title: "Chỉnh sửa", label: "Sửa", icon: <MessageSquare className="size-4" />, onClick: () => onToggleDialog(node) }] : []),
        ...(isText ? [{ id: "editText", title: "Sửa văn bản", label: "Sửa chữ", icon: <Pencil className="size-4" />, onClick: () => onEditText(node) }] : []),
        ...(isText ? [{ id: "generateImage", title: "Tạo ảnh từ văn bản", label: "Tạo ảnh", icon: <ImageIcon className="size-4" />, onClick: () => onGenerateImage(node) }] : []),
        ...(isConfig ? [{ id: "config", title: "Cấu hình tạo", label: "Cấu hình", icon: <Settings2 className="size-4" />, onClick: () => onToggleDialog(node) }] : []),
        ...(isText ? [{ id: "decreaseFont", title: "Giảm cỡ chữ", label: "Nhỏ", icon: <Minus className="size-4" />, onClick: () => onDecreaseFont(node) }] : []),
        ...(isText ? [{ id: "increaseFont", title: "Tăng cỡ chữ", label: "Lớn", icon: <Plus className="size-4" />, onClick: () => onIncreaseFont(node) }] : []),
        ...(isImage && !hasImage ? [{ id: "uploadImage", title: "Tải ảnh lên", label: "Tải ảnh", icon: <Upload className="size-4" />, onClick: () => onUpload(node) }] : []),
        ...(isVideo ? [{ id: "uploadVideo", title: hasVideo ? "Thay video" : "Tải video lên", label: hasVideo ? "Thay video" : "Tải video", icon: <Video className="size-4" />, onClick: () => onUpload(node) }] : []),
        ...(isAudio ? [{ id: "uploadAudio", title: hasAudio ? "Thay âm thanh" : "Tải âm thanh lên", label: hasAudio ? "Thay âm thanh" : "Tải âm thanh", icon: <Music2 className="size-4" />, onClick: () => onUpload(node) }] : []),
    ...
    return (
        <>
            ...
            >
                ...
                {hasImage ? <ToolbarAction id="more" title="Cấu hình công cụ nhanh" label="Thêm" icon={<Ellipsis className="size-4" />} active={imageToolSettingsOpen} onClick={openImageToolSettings} showLabel={showImageToolLabels} /> : null}
            </div>
            {hasImage ? (
                ...
                <div className="h-[56vh] min-h-[360px] text-sm">
                    {view === "info" ? (
                        <div className="thin-scrollbar h-full space-y-3 overflow-auto pr-1">
                            ...
                            <InfoRow
                                label="Loại"
                                value={node.type === CanvasNodeType.Text ? "Văn bản" : node.type === CanvasNodeType.Image ? "Ảnh" : node.type === CanvasNodeType.Video ? "Video" : node.type === CanvasNodeType.Audio ? "Âm thanh" : "Cấu hình tạo"}
                            />
