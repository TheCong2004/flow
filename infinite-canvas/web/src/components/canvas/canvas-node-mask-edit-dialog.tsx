...
export function CanvasNodeMaskEditDialog({ dataUrl, open, onClose, onConfirm }: { dataUrl: string; open: boolean; onClose: () => void; onConfirm: (payload: CanvasImageMaskEditPayload) => void }) {
    ...
    const submit = () => {
        ...
        if (!nextPrompt) return setError("Nhập mô tả chỉnh sửa");
        if (!canvas) return;
        if (!canvasHasPaint(canvas)) return setError("Hãy tô vùng cần sửa trước");
        ...
        <Modal title={null} open={open && Boolean(dataUrl)} onCancel={onClose} footer={null} width={980} centered destroyOnHidden>
            <div className="grid gap-5 lg:grid-cols-[minmax(360px,1fr)_320px]">
                ...
                <div className="flex min-h-[360px] flex-col gap-5">
                    <div>
                        <h2 className="text-xl font-semibold">Sửa cục bộ (mask)</h2>
                        <div className="mt-2 text-sm opacity-60">{image ? `${image.width} x ${image.height}px` : "Đang đọc"}</div>
                    </div>
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center justify-between text-sm">
                            <span className="font-medium opacity-75">Cỡ cọ</span>
                    ...
                    <div className="space-y-2">
                        <div className="text-sm font-medium opacity-75">Mô tả chỉnh sửa</div>
                        <Input.TextArea
                            rows={6}
                            value={prompt}
                            status={error && !prompt.trim() ? "error" : undefined}
                            placeholder="VD: đổi vùng chọn thành kim loại, giữ ánh sáng ảnh gốc"
                            onChange={(event) => {
