    ...
    return (
        <Modal title={null} open={open && Boolean(dataUrl)} onCancel={onClose} footer={null} width={820} centered destroyOnHidden>
            <div className="space-y-5">
                <div>
                    <h2 className="text-xl font-semibold">Phóng to ảnh</h2>
                </div>
                <div className="grid gap-6 md:grid-cols-[minmax(260px,1fr)_360px]">
                    <div className="rounded-xl border p-4">
                        ...
                        <div className="mt-3 flex items-center justify-between text-sm">
                            <span className="opacity-60">Ảnh nguồn</span>
                            <span className="font-semibold">{image ? `${image.width} x ${image.height} px` : "Đang đọc"}</span>
                        </div>
                    </div>
                    <div className="space-y-6 py-2">
                        <div className="space-y-2">
                            <div className="font-medium opacity-75">Độ phân giải đích</div>
                            <Segmented
                                block
                            ...
                            />
                            {image && !canUpscale ? <div className="text-xs font-medium text-[#ef4444]">{reachedMax ? "Ảnh đã đạt 4K, không cần phóng to" : "Ảnh đã đạt độ phân giải đích, không cần phóng to"}</div> : null}
                        </div>
                        <div className="space-y-2">
                            <div className="font-medium opacity-75">Thuật toán phóng to</div>
                            <Segmented
                                block
                        ...
                        <div className="rounded-xl border px-4 py-3 text-sm">
                            <div className="flex items-center justify-between">
                                <span className="opacity-60">Kích thước xuất</span>
                                <span className="font-semibold">{outputSize ? `${outputSize.width} x ${outputSize.height} px` : "Không rõ"}</span>
                            </div>
                        </div>
                    </div>
                </div>
