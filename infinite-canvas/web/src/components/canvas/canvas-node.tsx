...
import { CanvasNodeType, type CanvasNodeData, type Position, type TobyFlowExecutionStatus } from "@/types/canvas";
        ...
        <div className="flex h-full w-full flex-col overflow-hidden pt-8">
            ...
            {isEditingContent ? (
                ...
                <div className="thin-scrollbar block h-full w-full overflow-y-auto whitespace-pre-wrap break-words bg-transparent pl-4 pr-14 pt-0 pb-4 font-mono" style={textStyle} onWheel={(event) => event.stopPropagation()}>
                    {node.metadata?.content || <span style={{ color: theme.node.placeholder }}>Nhấp đúp để sửa văn bản</span>}
                </div>
            )}
        </div>
    );
...
function EmptyImageContent({ theme, isBatchRoot, batchCount, batchExpanded, batchOpening, batchRecovering, onToggleBatch }: NodeContentRendererProps) {
    const content = (
        <div className="flex h-full w-full flex-col items-center justify-center gap-3" style={{ color: theme.node.placeholder }}>
            ...
            <span className="text-[10px] tracking-[0.18em] opacity-50">NODE ẢNH TRỐNG</span>
        </div>
    );
    if (isBatchRoot)
        return (
    ...
    if (!node.metadata?.content)
        return (
            <div className="flex h-full w-full flex-col items-center justify-center gap-3" style={{ color: theme.node.placeholder }}>
                ...
                <span className="text-sm">Node video trống</span>
            </div>
        );
    ...
    if (!node.metadata?.content)
        return (
            <div className="flex h-full w-full flex-col items-center justify-center gap-2" style={{ color: theme.node.placeholder }}>
                ...
                <span className="text-sm">Node âm thanh trống</span>
            </div>
        );
    return (
        <div className="flex h-full w-full flex-col justify-center gap-3 px-4" style={{ background: theme.node.fill, color: theme.node.text }}>
            <div className="flex min-w-0 items-center gap-2 text-sm opacity-70">
                ...
                <span className="truncate">{node.title || "Âm thanh"}</span>
            </div>
        ...
        <BatchFrame batchCount={isBatchRoot ? batchCount : 0} batchExpanded={batchExpanded} batchOpening={batchOpening} batchRecovering={batchRecovering} onToggleBatch={onToggleBatch}>
            ...
            {isBatchRoot ? (
                <button
                    ...
                    aria-label={batchExpanded ? "Batch đã mở" : "Batch đã thu gọn"}
                    onClick={(event) => {
                        event.stopPropagation();
                        onToggleBatch?.();
                    }}
...
const EXECUTION_BADGE: Record<TobyFlowExecutionStatus, { label: string; color: string }> = {
    pending: { label: "", color: "" },
...
function TobyFlowNodeOverlay({ node }: { node: CanvasNodeData }) {
    const tileCache = useWorkflowEditorStore((state) => state.tileCache);
    const tf = node.metadata?.tobyflow;
    if (!tf) return null;

    const status = tf.executionStatus || "pending";
    const badge = EXECUTION_BADGE[status];
    const refIds = (tf.refFileIds || "")
        .split(",")
        .map((s) => s.trim())
        .filter(Boolean);

    const resolveRefThumbnail = (fileId: string) => {
    ...
    return (
        <>
            {badge.label ? (
                <span className="pointer-events-none absolute right-2 top-2 z-20 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white" style={{ background: badge.color }} data-canvas-no-zoom>
                    {badge.label}
                </span>
            ) : null}
            {refIds.length > 0 ? (
                <div className="pointer-events-none absolute inset-x-2 bottom-2 z-20 flex gap-1" data-canvas-no-zoom>
                    {refIds.map((id) => {
                        const src = resolveRefThumbnail(id);
                        const isUploading = id.startsWith("upload_") && !id.startsWith("upload_import_");
                        return (
                            <div key={id} className="size-8 overflow-hidden rounded-md border border-white/20 bg-black/30">
                                {src ? <img src={src} alt="" className="h-full w-full object-cover" /> : isUploading ? <div className="h-full w-full animate-pulse bg-white/20" /> : <div className="h-full w-full bg-white/10" />}
                            </div>
                        );
                    })}
                </div>
