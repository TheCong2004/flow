    ...
    return (
        <div className="absolute bottom-5 left-5 z-50" onMouseDown={(event) => event.stopPropagation()} onPointerDown={(event) => event.stopPropagation()}>
            <div className="flex h-14 items-center gap-1 rounded-xl border px-2 shadow-lg backdrop-blur" style={dockStyle}>
                <Tooltip title={isMiniMapOpen ? "Tắt bản đồ nhỏ" : "Mở bản đồ nhỏ"}>
                    <Button
                        type="text"
                        ...
                        aria-label={isMiniMapOpen ? "Tắt bản đồ nhỏ" : "Mở bản đồ nhỏ"}
                    />
                </Tooltip>
                <Tooltip title="Đặt lại vùng nhìn">
                    <Button type="text" className="!h-8 !w-8 !min-w-8 !p-0" style={{ color: theme.toolbar.item }} icon={<Focus className="size-4" />} onClick={onReset} aria-label="Đặt lại vùng nhìn" />
                </Tooltip>
            ...
            <Modal title="Phím tắt" open={shortcutsOpen} onCancel={() => setShortcutsOpen(false)} footer={null} centered>
                <div className="space-y-3 border-t pt-4 text-sm" style={{ borderColor: theme.node.stroke }}>
                    <Shortcut label="Kéo canvas" value="Di chuyển vùng nhìn" />
                    <Shortcut label="Con lăn" value="Phóng to/thu nhỏ canvas" />
                    <Shortcut label="Ctrl / Cmd + kéo" value="Chọn nhiều node bằng khung" />
                    <Shortcut label="Shift / Ctrl / Cmd + nhấp" value="Thêm vào vùng chọn" />
                    <Shortcut label="Ctrl / Cmd + C / V" value="Sao chép / dán node" />
