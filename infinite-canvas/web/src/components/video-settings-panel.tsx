...
const sizeOptions = [
    { value: "1280x720", label: "Ngang", width: 1280, height: 720 },
    { value: "720x1280", label: "Dọc", width: 720, height: 1280 },
    { value: "1024x1024", label: "Vuông", width: 1024, height: 1024 },
    { value: "1792x1024", label: "Rộng", width: 1792, height: 1024 },
    { value: "1024x1792", label: "Dọc dài", width: 1024, height: 1792 },
    ...
    return (
        <ImageSettingsTheme theme={theme}>
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                {showTitle ? <div className="text-lg font-semibold">Cài đặt video</div> : null}
                <SettingGroup title="Độ nét" color={theme.node.muted}>
                ...
                <SettingGroup title="Kích thước" color={theme.node.muted}>
                ...
                <SettingGroup title="Thời lượng" color={theme.node.muted}>
                    <div className="grid grid-cols-3 gap-2.5">
...
function SeedanceVideoSettingsPanel({ config, onConfigChange, theme, showTitle, className }: VideoSettingsPanelProps) {
    ...
    return (
        <ImageSettingsTheme theme={theme}>
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                {showTitle ? <div className="text-lg font-semibold">Cài đặt video</div> : null}
                <SettingGroup title="Độ phân giải" color={theme.node.muted}>
                    <div className="grid grid-cols-3 gap-2.5">
                    ...
                    {isSeedanceFastModel(model) ? <div className="text-[11px] leading-4 opacity-55">Model fast không hỗ trợ 1080p, hệ thống sẽ tự dùng 720p.</div> : null}
                </SettingGroup>
                <SettingGroup title="Tỷ lệ" color={theme.node.muted}>
                    <div className="grid grid-cols-3 gap-2.5">
                    ...
                    <div className="grid grid-cols-4 gap-2.5">
                        {seedanceDurationOptions.map((value) => (
                            <OptionPill key={value} selected={duration === value} theme={theme} onClick={() => onConfigChange("videoSeconds", String(value))}>
                                {value === -1 ? "Thông minh" : `${value}s`}
                            </OptionPill>
                        ))}
                    </div>
                    <NumberInput value={String(duration)} min={-1} max={15} theme={theme} onChange={(value) => onConfigChange("videoSeconds", value)} />
                </SettingGroup>
                <SettingGroup title="Đầu ra" color={theme.node.muted}>
                    <div className="grid gap-2 rounded-xl border p-2.5" style={{ borderColor: theme.node.stroke }}>
                        <SwitchRow label="Tạo âm thanh" checked={generateAudio} theme={theme} onChange={(checked) => onConfigChange("videoGenerateAudio", String(checked))} />
                        <SwitchRow label="Thêm watermark" checked={watermark} theme={theme} onChange={(checked) => onConfigChange("videoWatermark", String(checked))} />
                    </div>
                </SettingGroup>
            </div>
        </ImageSettingsTheme>
...
export function videoSizeLabel(value: string) {
    ...
    if (value === "adaptive" || value === "auto") return "Tự thích ứng";
...
export function videoSecondsLabel(value: string) {
    if (String(value).trim() === "-1") return "Thông minh";
    return `${value || "6"}s`;
}
