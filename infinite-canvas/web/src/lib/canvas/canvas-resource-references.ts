...
export function buildSlugMentionReferences(nodes: CanvasNodeData[], activeNodeId?: string) {
    return nodes
        .filter((node) => Boolean(node.metadata?.tobyflow?.slug))
        .map((node) => {
            const slug = node.metadata!.tobyflow!.slug!;
            ...
            return {
                ...
                previewUrl: node.metadata?.content || node.metadata?.tobyflow?.refThumbnails?.[Object.keys(node.metadata?.tobyflow?.refThumbnails || {})[0] || ""],
                text: node.type === CanvasNodeType.Text ? node.metadata?.content || node.metadata?.prompt : undefined,
                active: activeNodeId ? node.id === activeNodeId : true,
            } satisfies CanvasResourceReference;
        });
}

export function buildNodeMentionReferences(node: CanvasNodeData, nodes: CanvasNodeData[], connections: CanvasConnection[]) {
    const connectionRefs = labelResourceNodes(getMentionResourceNodes(node.id, nodes, connections), true);
    const slugRefs = buildSlugMentionReferences(nodes, node.id);
    if (!slugRefs.length) return connectionRefs;

    const seen = new Set(connectionRefs.map((ref) => ref.nodeId));
    const merged = [...connectionRefs];
