...
import { type CanvasAgentOp, type CanvasAgentSnapshot, summarizeCanvasAgentOps } from '@/lib/canvas/canvas-agent-ops'
...
const ONLINE_AGENT_PROMPT =
    '你是 Infinite Canvas 网页内置在线画布助手。当前画布 JSON 会随用户消息提供。首轮必须调用工具：只读问题调用 canvas_get_state，需要改动画布时调用和本地 Agent 一致的 infinite-canvas 工具。需要生成内容时直接调用 canvas_generate_text、canvas_generate_image、canvas_generate_video、canvas_generate_audio 或 canvas_create_generation_flow；需要精确批量操作时调用 canvas_apply_ops。不要输出 JSON ops，不要编造执行结果。工具参数涉及已有节点时必须使用当前画布 JSON 中真实存在的 id；缺少必要 id 或用户意图不明确时直接说明需要用户明确Chọn或说明，不要猜测。工具返回结果后，再根据真实结果回答用户。'
...
function toolDefinition(
    ...
    return {
        type: 'function',
        function: { name, description, parameters: { type: 'object', properties, required, additionalProperties: false }, strict }
    }
}
...
const ONLINE_AGENT_TOOLS: ResponseFunctionTool[] = [
    ...
    toolDefinition('canvas_create_text_node', '在当前画布创建单个Văn bản节点。', {
        text: { type: 'string' },
        x: { type: 'number' },
        y: { type: 'number' },
        title: { type: 'string' },
    ...
    toolDefinition(
        ...
        '批量创建Văn bản节点，适合生成标题、段落、脚本、说明等内容块。',
        {
            items: {
                type: 'array',
                minItems: 1,
    ...
    toolDefinition(
        ...
        '创建Cấu hình tạo节点，可指定 text/image/video/audio 模式和生成参数，可Chọn立即触发生成。',
        {
            prompt: { type: 'string' },
            mode: GENERATION_MODE_SCHEMA,
            title: { type: 'string' },
    ...
    toolDefinition(
        ...
        '创建提示词Văn bản节点和ẢnhCấu hình tạo节点，并自动连线，可Chọn立即触发生图。',
        {
            prompt: { type: 'string' },
            x: { type: 'number' },
            y: { type: 'number' },
    ...
    generationToolDefinition(
        ...
        '创建通用生成流程：提示词Văn bản节点、Cấu hình tạo节点、参考节点连线，可用于文案、生图、Video或Âm thanh。'
    ),
    generationToolDefinition('canvas_generate_text', '创建通用Văn bản生成流程并立即触发生成。', 'text'),
    generationToolDefinition('canvas_generate_image', '创建通用Ảnh生成流程并立即触发生成。', 'image'),
    generationToolDefinition('canvas_generate_video', '创建通用Video生成流程并立即触发生成。', 'video'),
    generationToolDefinition('canvas_generate_audio', '创建通用Âm thanh生成流程并立即触发生成。', 'audio'),
    toolDefinition(
        'canvas_update_node',
        'Cập nhật node基础字段或 metadata。',
    ...
    toolDefinition(
        'canvas_resize_node',
        '调整节点尺寸。',
        { id: { type: 'string' }, width: { type: 'number' }, height: { type: 'number' }, freeResize: { type: 'boolean' } },
        ['id', 'width', 'height']
    ),
    toolDefinition('canvas_delete_nodes', 'Xóa指定节点及相关连线。', { ids: { type: 'array', items: { type: 'string' }, minItems: 1 } }, [
        'ids'
    ]),
    toolDefinition(
    ...
    toolDefinition(
        ...
        '触发指定节点生成，通常用于配置节点或Văn bản/Ảnh/Video/Âm thanh节点。',
...
export function CanvasAssistantPanel({
    ...
    const activeSession = useMemo(
        () => safeSessions.find((session) => session.id === localActiveSessionId) || safeSessions[0] || null,
    ...
    const addOnlineLog = (title: string, data?: unknown) =>
        setOnlineLogs((prev) => [{ id: nanoid(), time: new Date().toLocaleTimeString(), title, data }, ...prev].slice(0, 80))
    ...
    const upsertMessage = (sessionId: string, message: CanvasAssistantMessage) => {
        updateSession(sessionId, (session) => {
            ...
            return {
                ...
                title: session.messages.length ? session.title : message.text.slice(0, 18) || '新对话',
                messages: exists
                    ? session.messages.map((item) => (item.id === message.id ? { ...item, ...message } : item))
            ...
            if (result.toolCalls.length) {
                ...
                if (confirmTools && writableCalls.length) {
                    upsertMessage(sessionId, {
                        id: assistantId,
                        role: 'assistant',
                        text: result.content || streamed || '准备执行工具，等待Xác nhận。'
                    })
                    ...
                    addOnlineLog('等待用户Xác nhận', result.toolCalls)
                    return
                }
    ...
    const continueOnlineToolLoopAfterResults = async (
        ...
        if (next.toolCalls.length) {
            ...
            if (confirmTools && writableCalls.length) {
                upsertMessage(sessionId, {
                    id: assistantId,
                    role: 'assistant',
                    text: next.content || streamed || '准备执行工具，等待Xác nhận。'
                })
                const toolMessageId = nanoid()
                pendingToolContextRef.current.set(toolMessageId, {
                ...
                addOnlineLog('等待用户Xác nhận', next.toolCalls)
                return
            }
        ...
        upsertMessage(sessionId, {
            id: assistantId,
            role: 'assistant',
            text: next.content || streamed || toolResults.map((item) => toolResultText(item.result)).join('\n') || '工具已执行。'
        })
    }
        ...
        try {
            ...
            if (name === 'canvas_get_selection') {
                ...
                return {
                    ...
                    data: { nodes: compactSnapshot({ ...current, nodes: current.nodes.filter((node) => ids.has(node.id)) }).nodes }
                }
            }
            ...
            return {
                ok: result.changed,
                message: result.changed ? summarizeCanvasAgentOps(ops) || '画布操作已执行。' : result.noopReason,
                data: result
            }
        } catch (error) {
    ...
    const rejectOnlineTool = (messageId: string) => {
        ...
        if (session)
            upsertMessage(session.id, {
                ...
                title: '已拒绝执行',
                text: '工具调用已Hủy',
                detail: { ...objectDetail(session.messages.find((item) => item.id === messageId)?.detail), status: 'rejected' }
            })
    }
    ...
    const onlineContent = (
        <>
                                ...
                                <AssistantReferenceChip
                                    ...
                                    onRemove={() => {
                                        ...
                                        if (selectedNodeIds.has(item.id))
                                            onSelectNodeIds(new Set(Array.from(selectedNodeIds).filter((nodeId) => nodeId !== item.id)))
                                    }}
                                />
                            ))}
                        </div>
            ...
            <Modal
                ...
                footer={
                    <>
                        <Button onClick={() => setDeleteChatIds([])}>Hủy</Button>
                        <Button
                            danger
                            type='primary'
                            onClick={() => {
            ...
            >
                <p className='text-sm opacity-60'>将Xóa {deleteChatIds.length} 条对话记录，此操作不可撤销。</p>
            </Modal>
        </>
    )
    ...
    return (
        <Select value={current} onValueChange={onChange}>
            <SelectTrigger
                ...
                title={current ? `${modelOptionName(current)} · ${resolveModelChannel(config, current).name}` : 'ChọnVăn bản模型'}
                onMouseDown={(event) => event.stopPropagation()}
                onPointerDown={(event) => event.stopPropagation()}
            >
                <AgentModelIcon model={current} />
                <span className='min-w-0 truncate'>{current ? modelOptionName(current) : 'ChọnVăn bản模型'}</span>
    ...
    if (name.includes('grok')) {
        try {
            return typeof chrome !== 'undefined' && chrome.runtime?.getURL
                ? chrome.runtime.getURL('assets/tobyflow/dist/infinite-canvas/icons/grok.svg')
                : new URL('dist/infinite-canvas/icons/grok.svg', document.baseURI).href
        } catch {
            return 'dist/infinite-canvas/icons/grok.svg'
        }
    }
            ...
            <div className='space-y-4'>
                <div>
                    ...
                    <div className='mt-1 text-xs leading-5' style={{ color: theme.node.muted }}>
                        网站 Agent 直接使用当前网页配置的Văn bản模型和 API。
                    </div>
                </div>
                <div className='rounded-lg border p-3' style={{ borderColor: theme.node.stroke }}>
                    <div className='flex flex-wrap items-start justify-between gap-3'>
                        <div className='min-w-0 flex-1'>
                            <div className='text-sm font-medium leading-5'>Văn bản模型</div>
...
function formatOnlineLogText(logs: OnlineAgentLog[], context: OnlineAgentLogContext) {
    ...
    const body = logs
        .map((log, index) =>
            [`#${index + 1} ${log.time} ${log.title}`, log.data === undefined ? '' : stringifyLog(log.data)].filter(Boolean).join('\n')
        )
        .join('\n\n---\n\n')
...
function describeCanvasSnapshot(snapshot: CanvasAgentSnapshot) {
    ...
    return `当前画布有 ${snapshot.nodes.length} 个节点、${snapshot.connections.length} 条连线。Văn bản ${
        counts[CanvasNodeType.Text] || 0
    } 个，Ảnh ${counts[CanvasNodeType.Image] || 0} 个，Cấu hình tạo ${counts[CanvasNodeType.Config] || 0} 个，Video ${
        counts[CanvasNodeType.Video] || 0
    } 个，Âm thanh ${counts[CanvasNodeType.Audio] || 0} 个。`
}
...
function onlineToolToOps(name: string, input: Record<string, unknown>, snapshot: CanvasAgentSnapshot, config: AiConfig): CanvasAgentOp[] {
    ...
    if (name === 'canvas_create_config_node') {
        ...
        return [
            configNodeOp(configId, input, numberOr(input.x, nextCanvasX(snapshot)), numberOr(input.y, 0), config),
    ...
    if (name === 'canvas_run_generation')
        return [runGenerationOp(requireString(input.nodeId, 'nodeId'), generationMode(input.mode), stringOptional(input.prompt))]
...
function configNodeOp(id: string, input: Record<string, unknown>, x: number, y: number, config: AiConfig): CanvasAgentOp {
    ...
    return {
        ...
        metadata: cleanRecord({
            ...
            count:
                numberOptional(input.count) ?? generationCount(mode === 'image' ? config.canvasImageCount || config.count : config.count),
...
function toolCallLabel(name: string) {
    ...
    if (name === 'canvas_create_node') return 'Tạo node'
    if (name === 'canvas_create_text_node') return '创建Văn bản'
    if (name === 'canvas_create_text_nodes') return '批量创建Văn bản'
    if (name === 'canvas_create_config_node') return '创建Cấu hình tạo'
    if (name === 'canvas_create_image_prompt_flow') return '创建生图流程'
    if (name === 'canvas_create_generation_flow') return '创建生成流程'
    if (name === 'canvas_generate_text') return '生成Văn bản'
    if (name === 'canvas_generate_image') return '生成Ảnh'
    if (name === 'canvas_generate_video') return '生成Video'
    if (name === 'canvas_generate_audio') return '生成Âm thanh'
    if (name === 'canvas_update_node') return 'Cập nhật node'
    if (name === 'canvas_update_node_text') return '更新Văn bản'
    if (name === 'canvas_move_nodes') return 'Di chuyển node'
    if (name === 'canvas_resize_node') return '调整节点尺寸'
    if (name === 'canvas_delete_nodes') return 'Xóa节点'
    if (name === 'canvas_connect_nodes') return 'Nối node'
    if (name === 'canvas_select_nodes') return 'Chọn节点'
...
function toCanvasAgentOp(value: unknown): CanvasAgentOp {
    ...
    if (type === 'delete_node')
        return { type, id: stringOptional(item.id), ids: Array.isArray(item.ids) ? requireStringArray(item.ids, 'ids') : undefined }
    ...
    if (type === 'run_generation')
        return { type, nodeId: requireString(item.nodeId, 'nodeId'), mode: generationMode(item.mode), prompt: stringOptional(item.prompt) }
...
function generationTitle(mode: 'text' | 'image' | 'video' | 'audio') {
    if (mode === 'text') return 'Văn bản生成'
    if (mode === 'video') return 'Video生成'
    if (mode === 'audio') return 'Âm thanh生成'
    return 'Ảnh生成'
}
...
function explainNoop(ops: CanvasAgentOp[], snapshot: CanvasAgentSnapshot) {
    ...
    const deleteConnectionOps = ops.filter(
        (op): op is Extract<CanvasAgentOp, { type: 'delete_connections' }> => op.type === 'delete_connections'
    )
    ...
    if (deleteConnectionOps.length && !snapshot.connections.length) return '画布当前没有连线可Xóa。'
    if (
        deleteConnectionOps.length &&
        deleteConnectionOps.every((op) => !op.all && [...(op.ids || []), ...(op.id ? [op.id] : [])].every((id) => !connectionIds.has(id)))
    )
        return '没有找到要Xóa的连线。'
    if (
        connectOps.length &&
        connectOps.every((op) => snapshot.connections.some((conn) => conn.fromNodeId === op.fromNodeId && conn.toNodeId === op.toNodeId))
    )
        return '这些节点已经存在对应连线，无需重复连接。'
    if (connectOps.length && connectOps.every((op) => !nodeIds.has(op.fromNodeId) || !nodeIds.has(op.toNodeId)))
        return '没有找到要连接的节点。'
    if (
        ...
        return '画布当前没有Cấu hình tạo节点可Xóa。'
    if (deleteNodeOps.length && deleteNodeOps.every((op) => [...(op.ids || []), ...(op.id ? [op.id] : [])].every((id) => !nodeIds.has(id))))
        return '没有找到要Xóa的节点。'
    if (updateOps.length && updateOps.every((op) => !nodeIds.has(op.id))) return '没有找到要更新的节点。'
    if (selectOps.length && selectOps.every((op) => !(op.ids || []).some((id) => nodeIds.has(id)))) return '没有找到要Chọn的节点。'
    ...
    if (selectOps.length && selectOps.every((op) => JSON.stringify(op.ids || []) === JSON.stringify(snapshot.selectedNodeIds)))
        return '选区已经是目标状态。'
            ...
            content: [
                ...
                ...(await Promise.all(
                    refs
                        ...
                        .map(async (item) => ({ type: 'image_url' as const, image_url: { url: await imageToDataUrl(item) } }))
                ))
            ]
        }
    ]
