    ...
    return (
        ...
        >
            <div className='mb-2 flex items-center justify-between gap-2'>
                <div className='flex min-w-0 items-baseline gap-2'>
                    ...
                    <div className='truncate text-[11px] opacity-55'>
                        Dùng @ để tham chiếu tài nguyên đã nối, tự đánh số lại trước khi gửi
                    </div>
                </div>
            ...
            {imagePreview ? (
                <Image
                    src={imagePreview}
                    alt='Xem trước ảnh tham chiếu'
                    style={{ display: 'none' }}
                    preview={{ visible: true, src: imagePreview, onVisibleChange: (visible) => !visible && setImagePreview(null) }}
                />
            ) : null}
        </div>
    )
...
function ResourcePreview({ input }: { input: NodeGenerationInput }) {
    ...
    if (input.type === 'video' && input.video)
        return <video src={input.video.url} className='size-9 rounded-md bg-black object-cover' muted preload='metadata' />
