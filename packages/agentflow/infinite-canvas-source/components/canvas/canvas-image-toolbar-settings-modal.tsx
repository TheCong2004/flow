...
import { Button, Card, Checkbox, Form, Modal, Space, Switch, Tag, theme as antdTheme, Tooltip, Typography } from 'antd'
...
export function ImageToolSettingsModal({
    ...
    const previewTools: PreviewTool[] = [
        ...selectedTools,
        { id: 'more', title: 'Cấu hình công cụ nhanh', label: 'Thêm', icon: <Ellipsis className='size-4' />, active: true }
    ]
    ...
    const scrollbarThumbWidth =
        previewScroll.max > 0
            ? Math.min(scrollbarWidth, Math.max(64, (previewScroll.viewport / previewScroll.content) * scrollbarWidth))
            : scrollbarWidth

    return (
        <Modal
            title='Tùy chỉnh thanh công cụ'
            open={open}
            centered
            width={760}
            onCancel={onCancel}
        ...
        >
            <Typography.Paragraph type='secondary' className='!mb-4'>
                Chọn công cụ nhanh trên thanh chỉnh sửa node ảnh.
            ...
            >
                <div className='relative flex min-h-[300px] w-full justify-center pt-20 pb-9'>
                    ...
                    >
                        ...
                        <Typography.Text type='secondary'>Node ảnh</Typography.Text>
                    </div>
                    <input
