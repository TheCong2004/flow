...
export function CanvasLocalAgentPanel({
    ...
    useEffect(() => {
        ...
        source.addEventListener('hello', () => {
            ...
            setAgentState({
                ...
                activity: '已连接',
                connectError: '',
                messages: useCanvasAgentStore.getState().messages.filter((item) => !isConnectionErrorMessage(item))
            })
    ...
    const sendPrompt = async () => {
        ...
        if (attachmentPayloadBytes(files) > MAX_ATTACHMENT_PAYLOAD_BYTES) {
            addMessage({ role: 'error', title: 'Ảnh过大', text: 'Ảnh附件超过 30MB，请删减后再发送。' })
            return
        }
        setAgentState({ activity: '发送中', sending: true, waiting: true })
        addMessage({ role: 'user', text: text || '发送了Ảnh', attachments: files })
    ...
    const addAttachments = async (files: FileList | File[] | null) => {
        ...
        try {
            ...
            if (attachmentPayloadBytes(merged) > MAX_ATTACHMENT_PAYLOAD_BYTES) {
                ...
                addMessage({ role: 'error', title: 'Ảnh过大', text: 'Ảnh附件最多约 30MB。' })
                return
            }
            if (next.length) setAgentState({ attachments: merged })
        } catch (error) {
            addMessage({ role: 'error', title: 'Ảnh读取失败', text: error instanceof Error ? error.message : 'Ảnh读取失败' })
        }
    }
        ...
        if (confirmToolsRef.current && payload.name === 'canvas_apply_ops') {
            if (pendingToolRef.current) {
                await postToolResult(endpoint, token, clientIdRef.current, {
                    ...
                    error: '仍有待Xác nhận的画布工具调用'
                })
                return
            }
            pendingToolRef.current = payload
            setAgentState({ pendingTool: payload, activity: '等待Xác nhận', waiting: false })
            addEventLog('等待Xác nhận', payload, payload)
            return
        }
    ...
    const rejectPendingTool = async () => {
        ...
        await postToolResult(endpoint, token, clientIdRef.current, { requestId: pendingTool.requestId, error: '用户Hủy了画布工具调用' })
        setAgentState({ activity: '已Hủy', waiting: false })
        addMessage({
            role: 'tool',
            title: '拒绝执行',
    ...
    const startNewThread = async () => {
        ...
        setAgentState({ loadingThreads: true })
        try {
            const data = await fetchAgentJson<AgentThreadResponse>(endpoint, token, '/agent/codex/threads/new', {
                method: 'POST',
                headers: { 'content-type': 'application/json' },
    ...
    const deleteThread = async (threadId: string) => {
        ...
        setAgentState({ loadingThreads: true })
        try {
            await fetchAgentJson(endpoint, token, `/agent/codex/threads/${encodeURIComponent(threadId)}/delete`, {
                method: 'POST',
                headers: { 'content-type': 'application/json' },
                body: JSON.stringify({ canvasId: projectId })
            })
            ...
            message.success('记录已Xóa')
        } catch (error) {
            addEventLog('Xóa对话失败', error)
            message.error(error instanceof Error ? error.message : 'Xóa对话失败')
        } finally {
            setAgentState({ loadingThreads: false })
        }
    }
    ...
    const confirmDeleteThread = (thread: AgentThreadSummary) => {
        ...
        modal.confirm({
            title: 'Xóa对话记录',
            content: `确定Xóa「${label.length > 48 ? `${label.slice(0, 48)}...` : label}」吗？`,
            okText: 'Xóa',
            okType: 'danger',
            cancelText: 'Hủy',
    ...
    const addMessage = (item: Omit<AgentChatItem, 'id'>) => {
            ...
            if (index >= 0) {
                setAgentState({
                    messages: currentMessages.map((message, i) =>
                        i === index ? { ...message, ...next, id: message.id, text: next.text || message.text } : message
                    )
                })
                return
            }
        ...
        if (last?.role === 'assistant' && next.role === 'assistant' && last.title === next.title) {
            ...
            setAgentState({
                messages: [...useCanvasAgentStore.getState().messages.slice(0, -1), { ...last, text: merged, meta: next.meta || last.meta }]
            })
            return
        }
        pushMessage(next)
...
function AgentLogView({
    ...
    const copy = async (value = content, tip = '日志Đã sao chép') => {
                ...
                <div className='flex flex-wrap items-center justify-between gap-2'>
                    ...
                    <div className='flex items-center gap-2'>
                        ...
                        <Button
                            size='small'
                            disabled={!lastError}
                            onClick={() => lastError && void copy(formatLogText([lastError], context), '最近错误Đã sao chép')}
                        >
                            最近错误
                        </Button>
                        <Button
                        ...
                        <div className='min-w-0 flex-1'>
                            <div className='flex min-w-0 items-center gap-2'>
                                ...
                                <span
                                    ...
                                    style={{
                                        borderColor: connected || enabled || connectError ? statusColor : theme.node.stroke,
                                        color: statusColor
                                    }}
                                >
...
async function postToolResult(
    ...
    await fetch(`${endpoint}/canvas/result?token=${encodeURIComponent(token)}&clientId=${encodeURIComponent(clientId)}`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
...
function formatAgentEvent(event: AgentEventPayload): Omit<AgentChatItem, 'id'> | null {
    ...
    if ((event.type === 'item.updated' || event.type === 'item.completed') && item?.type === 'agent_message')
        return { role: 'assistant', title: 'Codex', text: stringText(item.text), meta: usageText(event), streamId: item.id }
...
function promptWithAttachments(text: string, attachments: AgentAttachment[]) {
    ...
    return [text, `用户上传了 ${attachments.length} mảnhẢnh附件：${names}。`].filter(Boolean).join('\n\n')
}
...
function readDataUrl(file: File) {
    return new Promise<string>((resolve, reject) => {
        ...
        reader.onerror = () => reject(reader.error || new Error('读取Ảnh失败'))
