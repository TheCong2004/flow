...
export function CanvasNodeCropDialog({
    ...
    const cropSize = image
        ? { width: Math.max(1, Math.round(crop.width * image.width)), height: Math.max(1, Math.round(crop.height * image.height)) }
        : null
    ...
    return (
        <Modal title='Cắt ảnh' open={open && Boolean(dataUrl)} onCancel={onClose} footer={null} width={780} centered destroyOnHidden>
            <div className='space-y-4'>
                        ...
                        >
                            ...
                            {handles.map((handle) => (
                                <button
                                    ...
                                    aria-label='Điều chỉnh khung cắt'
                                />
                            ))}
                        </div>
                    </div>
                ...
                <div className='flex flex-wrap items-center justify-between gap-3 rounded-lg border px-3 py-2'>
                    <div className='flex flex-wrap items-center gap-3 text-sm opacity-80'>
                        <span>Kích thước cắt {cropSize ? `${cropSize.width} x ${cropSize.height}` : 'Không rõ'}</span>
                        <span>Tỉ lệ {cropSize ? formatRatio(cropSize.width, cropSize.height) : 'Không rõ'}</span>
                        {image ? (
                            <span>
                                Ảnh gốc {image.width} x {image.height}
                            </span>
                        ) : null}
                    </div>
                    <Button
                    ...
                    >
                        {locked ? 'Khóa tỉ lệ' : 'Tỉ lệ tự do'}
                    </Button>
                </div>

                <div className='flex items-center justify-end gap-2'>
                    <Button onClick={() => setCrop(defaultCrop)}>Đặt lại</Button>
...
function CropMask({ crop }: { crop: CanvasImageCropRect }) {
    return (
        <>
            ...
            <div
                className='absolute bg-black/55'
                style={{ right: 0, top: `${crop.y * 100}%`, width: `${(1 - crop.x - crop.width) * 100}%`, height: `${crop.height * 100}%` }}
            />
        </>
    )
}
