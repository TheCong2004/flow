...
import { type CanvasNodeData, CanvasNodeType, type ViewportTransform } from '@/types/canvas'

import { type ImageToolbarSettingsTool, ImageToolSettingsModal } from './canvas-image-toolbar-settings-modal'
import {
...
export function CanvasNodeHoverToolbar({
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...(canRetry
            ? [{ id: 'retry', title: 'Tạo lại', label: 'Thử lại', icon: <RefreshCw className='size-4' />, onClick: () => onRetry(node) }]
            : []),
        ...(hasImage || hasVideo || isText
            ? [
                  {
                      id: 'saveAsset',
                      title: 'Thêm vào tài nguyên của tôi',
                      label: 'Lưu',
        ...
        ...(hasImage || hasVideo || hasAudio
            ? [
                  {
                      id: 'download',
                      title: hasAudio ? 'Tải âm thanh' : hasVideo ? 'Tải video' : 'Tải ảnh',
                      label: 'Tải',
        ...
        ...(isVideo
            ? [
                  {
                      id: 'uploadVideo',
                      title: hasVideo ? 'Thay video' : 'Tải video lên',
                      label: hasVideo ? 'Thay video' : 'Tải video',
        ...
        ...(isAudio
            ? [
                  {
                      id: 'uploadAudio',
                      title: hasAudio ? 'Thay âm thanh' : 'Tải âm thanh lên',
                      label: hasAudio ? 'Thay âm thanh' : 'Tải âm thanh',
        ...
        >
            ...
            >
                <span
                    ...
                    } rounded-lg transition group-hover:bg-[#f0f0f1] ${active ? 'bg-[#eeeeef]' : ''}`}
                >
                    {icon}
