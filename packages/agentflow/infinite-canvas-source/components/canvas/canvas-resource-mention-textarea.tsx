...
export const CanvasResourceMentionTextarea = forwardRef<HTMLTextAreaElement, Props>(function CanvasResourceMentionTextarea(
    ...
    const activeLabels = useMemo(
        () =>
            highlightLabels
                ? Array.from(new Set(references.filter((item) => item.active).map((item) => item.label))).sort(
...
function ReferencePreview({ reference }: { reference: CanvasResourceReference }) {
    ...
    if (reference.kind === 'video' && reference.previewUrl)
        return <video src={reference.previewUrl} className='size-9 rounded-md bg-black object-cover' muted preload='metadata' />
    const Icon =
        reference.kind === 'audio' ? Music2 : reference.kind === 'video' ? Video : reference.kind === 'image' ? ImageIcon : FileText
    return (
