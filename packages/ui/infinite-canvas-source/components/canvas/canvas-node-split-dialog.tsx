...
export function CanvasNodeSplitDialog({
    ...
    const pieceSize = image
        ? { width: Math.max(1, Math.floor(image.width / params.columns)), height: Math.max(1, Math.floor(image.height / params.rows)) }
        : null
    ...
    return (
        <Modal title={null} open={open && Boolean(dataUrl)} onCancel={onClose} footer={null} width={780} centered destroyOnHidden>
            <div className='space-y-5'>
                <div>
                    <h2 className='text-xl font-semibold'>Chia ảnh</h2>
                    <p className='mt-1 text-sm opacity-60'>生成 {total} 个Ảnh子节点，并按Ảnh gốc网格排列到画布右侧</p>
                </div>
                <div className='grid gap-6 md:grid-cols-[minmax(260px,1fr)_280px]'>
                    <div className='rounded-xl border p-4'>
                        ...
                        <div className='mt-3 flex items-center justify-between text-sm'>
                            <span className='opacity-60'>Ảnh gốc</span>
                            <span className='font-semibold'>{image ? `${image.width} x ${image.height} px` : 'Đang đọc'}</span>
                        </div>
                    </div>
                    <div className='space-y-5 py-2'>
                        <NumberField label='Số hàng' value={params.rows} onChange={(value) => update('rows', value)} />
                        <NumberField label='Số cột' value={params.columns} onChange={(value) => update('columns', value)} />
                        <div className='rounded-xl border px-4 py-3 text-sm'>
                            ...
                            <div className='mt-2 flex items-center justify-between'>
                                ...
                                <span className='font-semibold'>{pieceSize ? `${pieceSize.width} x ${pieceSize.height}` : 'Không rõ'}</span>
                            </div>
                        </div>
                        <Button
                            type='primary'
