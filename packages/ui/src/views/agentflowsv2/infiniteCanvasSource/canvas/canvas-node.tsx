    ...
    return (
        ...
        >
            ...
            {showPanel && renderPanel ? (
                <div className='absolute left-1/2 top-full z-[70] w-[500px] -translate-x-1/2 pt-4'>{renderPanel(data)}</div>
            ) : null}
        </div>
    )
})
            ...
            <div className='h-full w-full overflow-hidden rounded-3xl'>
                <img
                    ...
                    className={`pointer-events-none block h-full w-full select-none ${
                        node.metadata?.freeResize ? 'object-fill' : 'object-contain'
                    }`}
                />
            </div>
            {isBatchRoot ? (
...
function TobyFlowNodeOverlay({ node }: { node: CanvasNodeData }) {
    ...
    return (
        <>
            {badge.label ? (
                <span
                    className='pointer-events-none absolute right-2 top-2 z-20 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white'
